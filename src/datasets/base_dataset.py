...
def get_data(path, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None, train_file='train.txt', test_file='test.txt'):
    ...
    trn_lines = np.loadtxt(os.path.join(path, train_file), dtype=str)
    tst_lines = np.loadtxt(os.path.join(path, test_file), dtype=str)
